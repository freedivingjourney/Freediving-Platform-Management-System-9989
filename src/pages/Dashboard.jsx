...
const { FiActivity, FiTarget, FiCalendar, FiTrendingUp, FiCompass, FiEdit3, FiHeart, FiUsers, FiShield, FiMapPin, FiClock } = FiIcons;
...
const Dashboard = () => {
  ...
  const daysSinceLastDive = lastDive ? 
    Math.floor((new Date() - new Date(lastDive.date)) / (1000 * 60 * 60 * 24)) : 'N/A';
  ...
  const mostRecordedDiscipline = Object.entries(disciplineCounts).length > 0 ? 
    Object.entries(disciplineCounts).sort(([,a], [,b]) => b - a)[0][0] : 'N/A';
  ...
  const favoriteSite = Object.entries(siteCounts).length > 0 ? 
    Object.entries(siteCounts).sort(([,a], [,b]) => b - a)[0][0] : 'N/A';
  ...
  const favoriteBuddy = Object.entries(buddyCounts).length > 0 ? 
    Object.entries(buddyCounts).sort(([,a], [,b]) => b - a)[0][0] : 'N/A';
  ...
  const mostCommonExperience = Object.entries(experienceCounts).length > 0 ? 
    Object.entries(experienceCounts).sort(([,a], [,b]) => b - a)[0][0] : 'N/A';
  ...
  const quickActions = [
    ...
    { name: 'Goal Tracker', path: '/goals', icon: FiTarget, color: 'yellow', description: 'Manage your objectives' },
    { name: 'Human Design', path: '/human-design', icon: FiHeart, color: 'indigo', description: 'Discover your learning style' }
  ];
              ...
              {recentActivity.length > 0 ? recentActivity.map((activity, index) => (
                <div key={index} className="flex items-start space-x-4 p-4 bg-gray-50 rounded-lg">
                  <div className="w-10 h-10 bg-ocean-100 rounded-lg flex items-center justify-center">
                    <SafeIcon 
                      icon={activity.type === 'dive' ? FiActivity : FiTarget} 
