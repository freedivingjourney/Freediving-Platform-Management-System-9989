...
const {
  FiActivity, FiTarget, FiCalendar, FiTrendingUp, FiCompass, FiEdit3, FiHeart,
  FiUsers, FiShield, FiMapPin, FiClock, FiWind, FiApple
} = FiIcons;
...
const Dashboard = () => {
  ...
  const mostRecordedDiscipline = Object.entries(disciplineCounts).length > 0 
    ? Object.entries(disciplineCounts).sort(([,a], [,b]) => b - a)[0][0] 
    : 'N/A';
  ...
  const favoriteBuddy = Object.entries(buddyCounts).length > 0 
    ? Object.entries(buddyCounts).sort(([,a], [,b]) => b - a)[0][0] 
    : 'N/A';
  ...
  const mostCommonExperience = Object.entries(experienceCounts).length > 0 
    ? Object.entries(experienceCounts).sort(([,a], [,b]) => b - a)[0][0] 
    : 'N/A';
  ...
  const quickActions = [
    ...
    { name: 'Nutrition Plans', path: '/freediving-diets', icon: FiApple, color: 'yellow', description: 'Optimize your nutrition' },
    { name: 'Human Design', path: '/human-design', icon: FiHeart, color: 'emerald', description: 'Discover your learning style' }
  ];
